...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_threads; i++)
    {
        ...
        // if there is a remainder, add one to the range, with this policy we prioritize more tasks to the first threads
        ...
        printf("--Cria a thread %d com indices {%d,%d} - %d posicoes\n", i, args->startPos, args->endPos, args->endPos - args->startPos + 1);
